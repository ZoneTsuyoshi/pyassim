...
class MarkovTransitionKalmanFilter(object) :
    ...
    Args:
        ...
        transition_matrices [n_components, n_dim_sys, n_dim_sys] or [n_dim_sys, n_dim_sys]
            {numpy-array, float}
            also known as :math:`F`. transition matrix from x_{t-1} to x_{t}
            システムモデルの変換行列[離散状態軸，状態変数軸，状態変数軸] or [状態変数軸，状態変数軸]
        observation_matrices [n_components, n_dim_sys, n_dim_obs] or [n_dim_sys, n_dim_obs]
            {numpy-array, float}
            also known as :math:`H`. observation matrix from x_{t} to y_{t}
            観測行列[離散状態軸，状態変数軸，観測変数軸] or [状態変数軸，観測変数軸]
        transition_covariance [n_components, n_dim_sys, n_dim_sys] or [n_dim_sys, n_dim_sys]
            {numpy-array, float}
            also known as :math:`Q`. system transition covariance for times
            システムノイズの共分散行列[離散状態軸，状態変数軸，状態変数軸] or [状態変数軸，状態変数軸]
        observation_covariance [n_components, n_dim_obs, n_dim_obs] or [n_dim_obs, n_dim_obs]
            {numpy-array, float} 
            also known as :math:`R`. observation covariance for times.
            観測ノイズの共分散行列[離散状態軸，観測変数軸，観測変数軸] or [観測変数軸，観測変数軸]
        transition_offsets [n_components, n_dim_sys] or [n_dim_sys] {numpy-array, float} 
            also known as :math:`b`. system offset for times.
            システムモデルの切片（バイアス，オフセット）[離散状態軸，状態変数軸] or [状態変数軸]
        observation_offsets [n_components, n_dim_obs] or [n_dim_obs]{numpy-array, float}
            also known as :math:`d`. observation offset for times.
            観測モデルの切片[離散状態軸，観測変数軸] or [観測変数軸]
        initial_probability [n_components] {numpy-array, float}:
            also known as :math:`q0`. initial probability distribution of discrete state.
            summention for axis=0 needs be 1.
        transition_kernel [n_components, n_components] {numpy-array, float}:
            also known as :math:`P`. transition kernel for Markov transition.
            summention for each column needs be 1.
        n_dim_sys {int}
    ...
    def __init__(self, observation = None,
                ...
                n_components = 3, n_dim_sys = None, n_dim_obs = None,
                dtype = xp.float32):
        ...
        if initial_probability is None :
            self.q = xp.ones(n_components, dtype=dtype) / n_components
        else:
            if xp.any(initial_probability<0):
                raise ValueError("probability needs be positive.")
            elif xp.sum(initial_probability)==0:
                raise ValueError("summention for probability needs be positive.")
            else:
                self.q = initial_probability / xp.sum(initial_probability)

        if transition_kernel is None :
            self.P = xp.ones([n_components, n_components], dtype=dtype)
        else:
            if transition_kernel.shape != (n_components, n_components):
                raise ValueError("shape of transition kernel must be ({},{})".format(n_components, 
                                                                                n_components))
            elif xp.any(transition_kernel<0):
                raise ValueError("transition kernel needs be positive.")
            elif xp.any(xp.sum(transition_kernel, axis=0) == 0):
                raise ValueError("summention for probability needs be positive.")
            else:
                self.P = transition_kernel / xp.sum(transition_kernel, axis=0)
    ...
    def _predict_update(self, t):
        ...
        """
        # probability of discrete distribution
        self.q = self.P @ self.q

        for i in range(self.n_components):
            # extract parameters for component i
            F = _last_dims(self.F, i, 2)
            Q = _last_dims(self.Q, i, 2)
            b = _last_dims(self.b, i, 1)

            # calculate predicted distribution for time t
            self.x_pred[t] += self.q[i] * (F @ self.x_filt[t-1] + b)
            self.V_pred[t] += self.q[i] * (F @ self.V_filt[t-1] @ F.T + Q)
    ...
    def _filter_update(self, t):
        ...
        for i in range(self.n_components):
            # extract parameters for component i
            H = _last_dims(self.H, i, 2)
            R = _last_dims(self.R, i, 2)
            d = _last_dims(self.d, i, 1)

            # calculate filter step
            K = self.V_pred[t] @ (
                H.T @ linalg.pinv(H @ (self.V_pred[t] @ H.T) + R)
                )
            self.x_filt[t] += self.q[i] * K @ (
                self.y[t] - (H @ self.x_pred[t] + d)
                )
            self.V_filt[t] -= self.q[i] * K @ (H @ self.V_pred[t])
