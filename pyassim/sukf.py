...
class SequentialUpdateKalmanFilter(object) :
    ...
    def forward(self):
        ...
        for t in range(T) :
            ...
            if xp.any(xp.ma.getmask(self.y[t])) :
                ...
                if t!=0:
                    if not xp.any(xp.ma.getmask(self.y[t-1])) and t%self.update_interval==0:
    ...
    def _update_transition_matrix(self, t):
        ...
        Y  = xp.outer(self.y[t-1], self.y[t])
        Fh = linalg.pinv(Hb) @ linalg.pinv(Y) @ xp.diag(xp.multiply(self.y[t], self.y[t])) @ H
